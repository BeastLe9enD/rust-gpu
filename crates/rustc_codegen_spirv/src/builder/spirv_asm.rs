            ...
            (OperandKind::LiteralContextDependentNumber, Some(word)) => {
                ...
                fn parse(ty: SpirvType<'_>, w: &str) -> Result<dr::Operand, String> {
                    ...
                    Ok(match ty {
                        ...
                        SpirvType::Float(32) => dr::Operand::LiteralBit32(
                            w.parse::<f32>().map(f32::to_bits).map_err(fmt)?,
                        ),
                        SpirvType::Float(64) => dr::Operand::LiteralBit64(
                            w.parse::<f64>().map(f64::to_bits).map_err(fmt)?,
                        ),
...
pub const IMAGE_OPERANDS: &[(&str, ImageOperands)] = &[
    ...
    ("NonTemporal", ImageOperands::NONTEMPORAL),
    ("Offsets", ImageOperands::OFFSETS),
];
pub const FP_FAST_MATH_MODE: &[(&str, FPFastMathMode)] = &[
    ...
    (
        ...
        FPFastMathMode::ALLOW_CONTRACT_FAST_INTEL,
    ),
    ("AllowReassocINTEL", FPFastMathMode::ALLOW_REASSOC_INTEL),
];
...
pub const LOOP_CONTROL: &[(&str, LoopControl)] = &[
    ...
    (
        ...
        LoopControl::INITIATION_INTERVAL_INTEL,
    ),
    ("MaxConcurrencyINTEL", LoopControl::MAX_CONCURRENCY_INTEL),
    ("DependencyArrayINTEL", LoopControl::DEPENDENCY_ARRAY_INTEL),
    ("PipelineEnableINTEL", LoopControl::PIPELINE_ENABLE_INTEL),
    ("LoopCoalesceINTEL", LoopControl::LOOP_COALESCE_INTEL),
    ("MaxInterleavingINTEL", LoopControl::MAX_INTERLEAVING_INTEL),
    (
        "SpeculatedIterationsINTEL",
        LoopControl::SPECULATED_ITERATIONS_INTEL,
    ),
    ("NoFusionINTEL", LoopControl::NO_FUSION_INTEL),
];
pub const FUNCTION_CONTROL: &[(&str, FunctionControl)] = &[
    ...
    ("OptNoneINTEL", FunctionControl::OPT_NONE_INTEL),
];
...
pub const MEMORY_ACCESS: &[(&str, MemoryAccess)] = &[
    ...
    (
        ...
        MemoryAccess::ALIAS_SCOPE_INTEL_MASK,
    ), //TODO:
    ("NO_ALIAS_INTEL_MASK", MemoryAccess::NO_ALIAS_INTEL_MASK), //TODO:
];
...
pub const RAY_FLAGS: &[(&str, RayFlags)] = &[
    ...
    (
        ...
        RayFlags::FORCE_OPACITY_MICROMAP2_STATE_EXT,
    ),
];
